...
var (
	ErrMissingPaddingByte    = errors.New("iff: missing padding byte")
	ErrMissingIFFChunkHeader = errors.New("iff: missing IFF chunk header")
	ErrListSubchunkTooLong   = errors.New("iff: list subchunk too long")
	ErrShortChunkData        = errors.New("iff: short chunk data")
	ErrShortChunkHeader      = errors.New("iff: short chunk header")
	ErrStaleReader           = errors.New("iff: stale reader")
	ErrSeekingUnsupported    = errors.New("iff: resource does not support seeking")
	ErrInvalidSeekWhence     = errors.New("iff: invalid seek whence")
	ErrSeekOutOfRange        = errors.New("iff: seek out of range")
)

func u32(b []byte) uint32 {
	return uint32(b[3]) | uint32(b[2])<<8 | uint32(b[1])<<16 | uint32(b[0])<<24 // Big Endian (I think)
}
...
// NewReader creates a new [Reader] for an IFF stream.
func NewReader(r io.Reader) (formType FourCC, data *Reader, err error) {
	var buf [chunkHeaderSize]byte
	if _, err := io.ReadFull(r, buf[:]); err != nil {
		if err == io.EOF || err == io.ErrUnexpectedEOF {
			err = ErrMissingIFFChunkHeader
		}
		return FourCC{}, nil, err
	}
	if !bytes.Equal(buf[:4], IffID[:]) {
		return FourCC{}, nil, ErrMissingIFFChunkHeader
	}
